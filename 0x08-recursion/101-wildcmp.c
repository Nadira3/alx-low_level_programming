...
int _strlen(char *s)
{
	...
	return (1 + _strlen(s + 1)); /* increments the length by once with every true evaluation of s */
}
/*
...
int wildcmp(char *s1, char *s2)
{
	if (*s2 == '*' && _strlen(s2) == 1) /* checks if s2 == "*" */
		return (1);
	if (!*s1 && !*s2)
/* checks if s1 & s2 both point to null signifying the end of both strings */
		return (1); /* returns 1 bcos both strings have matched all previous characters till the end */
	if (*s2 == '*') /* checks if s2 is * in the middle or start of a string */
	{
		if (*s1 == *s2 || *s1 != '\0')
/* checks if the corresponding s1 character matches the s2 character or s1 is not null */
		{
